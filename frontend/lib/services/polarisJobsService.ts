...
async function backfillLegacyJobsForUser(): Promise<void> {
  try {
    ...
    const { data: summaries, error: summariesErr } = await supabase
      ...
      .select(
        'id, company_name, report_title, summary_content, prelim_report, dynamic_questionnaire_report, greeting_report, org_report, requirement_report, created_at'
      )
      .eq('user_id', user.id);
    ...
    const imported = new Set<string>(
      (existingJobs || []).map((j: any) => j.legacy_summary_id).filter(Boolean)
    );
...
export async function updateJobStageData(
  ...
  try {
    ...
    const { error } = await supabase.from('polaris_jobs').update(updateData).eq('id', jobId);
...
export async function saveJobReport(
  ...
  try {
    ...
    const { error: reportError } = await supabase.from('polaris_job_reports').insert({
      job_id: jobId,
      report_type: reportType,
      content,
      metadata: metadata || {},
...
export async function editJobReport(
  ...
  try {
    ...
    await supabase.from('polaris_job_reports').insert({
      job_id: jobId,
      report_type: reportType,
...
export async function deleteJob(jobId: string): Promise<{ error: any }> {
  try {
    const { error } = await supabase.from('polaris_jobs').delete().eq('id', jobId);
...
async function logActivity(
  ...
  try {
    ...
    await supabase.from('polaris_job_activity').insert({
      job_id: jobId,
      user_id: user.id,
      action,
      stage,
 ...
 */
export function getJobReportContent(job: PolarisJob, reportType: ReportType): string | null {
 ...
 */
export function isReportEdited(job: PolarisJob, reportType: ReportType): boolean {
  const editedField = `${reportType}_report_edited` as keyof PolarisJob;
