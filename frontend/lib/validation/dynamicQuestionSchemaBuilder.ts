    ...
    case 'enhanced_scale':
      if (question.scaleConfig) {
        schema = z.number({
          ...
          .min(question.scaleConfig.min, `Please select a value of ${question.scaleConfig.min} or higher`)
          .max(question.scaleConfig.max, `Please select a value of ${question.scaleConfig.max} or lower`);
      } else {
        schema = z.number().int();
      }
      break;
    ...
    case 'number_spinner':
      ...
      if (question.numberConfig) {
        ...
        if (question.numberConfig.step && question.numberConfig.step !== 1) {
          schema = schema.refine(
            (val) => {
              const steps = (val - (question.numberConfig!.min || 0)) / question.numberConfig!.step!;
              return steps === Math.floor(steps);
            },
      ...
      switch (rule.rule) {
        case 'minLength':
          if (schema instanceof z.ZodString && typeof rule.value === 'number') {
            schema = schema.min(rule.value, rule.message || `Must be at least ${rule.value} characters`);
          }
          break;
          
        case 'maxLength':
          if (schema instanceof z.ZodString && typeof rule.value === 'number') {
            schema = schema.max(rule.value, rule.message || `Must be at most ${rule.value} characters`);
          }
          break;
          
        case 'minSelections':
          if (schema instanceof z.ZodArray && typeof rule.value === 'number') {
            schema = schema.min(rule.value, rule.message || `Select at least ${rule.value} options`);
          }
          break;
...
export function getSectionValidationStatus(
  ...
  return sections.map(section => {
    ...
    const sectionRequiredIds = section.questions
      .filter(q => q.required)
      .map(q => q.id);
