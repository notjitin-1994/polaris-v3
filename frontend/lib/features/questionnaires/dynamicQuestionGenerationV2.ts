...
const LLM_CONFIG = {
  claude: {
    ...
    maxTokens: 20000, // Increased to 20K to GUARANTEE all 10 sections complete (Claude Sonnet 4.5 supports up to 24K output tokens)
    temperature: 0.7,
  },
  perplexity: {
    ...
    maxTokens: 16000, // Increased to handle full responses
    temperature: 0.7,
  },
...
async function callClaude(
  ...
  try {
    ...
    if (response.stop_reason === 'max_tokens') {
      ...
      throw new Error(
        `Claude response was truncated at max_tokens (${config.maxTokens}). The generated JSON is incomplete. Please increase max_tokens or reduce the complexity of the request.`
      );
    }

    logger.info('claude.response.complete', 'Claude response completed successfully', {
...
function repairJSON(jsonString: string): string {
  ...
  // This handles cases where Claude was cut off mid-property
