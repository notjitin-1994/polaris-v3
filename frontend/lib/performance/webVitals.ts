 ...
 */
function getMetricRating(metric: keyof typeof RATING_THRESHOLDS, value: number): 'good' | 'needs-improvement' | 'poor' {
...
export function useWebVitals(): WebVitalsMetrics {
  ...
  useEffect(() => {
    ...
    import('web-vitals').then(({ getLCP, getFID, getCLS, getFCP, getTTFB, getINP }) => {
      // Largest Contentful Paint
      getLCP((metric) => {
        setMetrics(prev => ({
          ...prev,
