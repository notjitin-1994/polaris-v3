...
export function useSwipeGesture(options: UseSwipeGestureOptions = {}) {
  ...
  const calculateVelocity = useCallback((currentPos: { x: number; y: number }, lastPos: { x: number; y: number }, timeDelta: number) => {
  ...
  const calculateDirection = useCallback((startPos: { x: number; y: number }, currentPos: { x: number; y: number }) => {
  ...
  const calculateDistance = useCallback((startPos: { x: number; y: number }, currentPos: { x: number; y: number }) => {
    const deltaX = currentPos.x - startPos.x
    const deltaY = currentPos.y - startPos.y
    return Math.sqrt(deltaX * deltaX + deltaY * deltaY)
  }, [])

  const applyRubberBandEffect = useCallback((offset: number, maxOffset: number) => {
  ...
  const handleTouchMove = useCallback((event: TouchEvent) => {
    ...
    const hasExceededThreshold = Math.abs(offset) > threshold || Math.abs(velocity.x) > velocityThreshold
  ...
  const handleTouchEnd = useCallback((event: TouchEvent) => {
    ...
    const shouldComplete =
      gestureState.hasExceededThreshold ||
