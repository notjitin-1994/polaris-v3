  ...
  try {
    ...
    try {
      const subscriptionData = {
        ...
        status: razorpaySubscription.status,
        plan_name:
          planDetails?.item?.name || razorpaySubscription.plan?.name || `${tier} (${billingCycle})`,
        plan_amount: planDetails?.item?.amount || razorpaySubscription.plan?.amount || planAmount,
        plan_currency: planDetails?.item?.currency || razorpaySubscription.plan?.currency || 'INR',
        plan_period: billingCycle,
        plan_interval: billingCycle === 'monthly' ? 1 : 12, // 1 month or 12 months
        subscription_tier: tier,
        start_date: razorpaySubscription.current_start
      ...
      const responseData = {
        ...
        subscription: {
          ...
          planName:
            ...
            razorpaySubscription.plan?.name ||
            `${tier} (${billingCycle})`,
          planAmount: planDetails?.item?.amount || razorpaySubscription.plan?.amount || planAmount,
          planCurrency: planDetails?.item?.currency || razorpaySubscription.plan?.currency || 'INR',
          billingCycle,
          nextBillingDate: razorpaySubscription.current_end
            ? new Date(razorpaySubscription.current_end * 1000).toISOString()
            : null,
