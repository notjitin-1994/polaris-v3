...
function StaticWizardContent(): React.JSX.Element {
  ...
  const performAutosave = async (formData: StaticQuestionnaireFormData) => {
    ...
    if (!hasData) {
      console.log('Autosave skipped - no meaningful form data');
      return;
    }
  ...
  useEffect(() => {
    const subscription = watch((data) => {
      if (data && Object.keys(data).length > 0) {
        ...
        if (hasAnyData) {
          debouncedAutosave(data as StaticQuestionnaireFormData);
        }
      }
    });
  ...
  const onSubmit = async (data: StaticQuestionnaireFormData) => {
    ...
    try {
      ...
      console.log('  • Fallback: Claude (claude-sonnet-4-20250514)');
