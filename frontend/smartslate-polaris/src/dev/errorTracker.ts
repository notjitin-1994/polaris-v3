...
function toEntry(
  error: unknown,
  opts: Partial<Omit<ErrorLogEntry, 'id' | 'timestamp' | 'message' | 'type'>> & {
...
export function reportError(
  error: unknown,
  options: Partial<Pick<ErrorLogEntry, 'type' | 'origin' | 'tags' | 'statusCode' | 'code'>> & {
    context?: any;
  } = {}
) {
    ...
    console.warn = (...args: any[]) => {
      try {
        breadcrumbs.add({
          category: 'console',
          level: 'warning',
          message: String(args[0] ?? 'warn'),
          data: { args: args.slice(1) },
        });
      } catch {}
      try {
        (originalConsoleWarn as any)(...args);
      } catch {}
    };
  } catch {}
}
