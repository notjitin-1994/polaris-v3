...
export default function PolarisJobViewer() {
  ...
  async function loadJobData() {
    try {
      ...
      setTitleInput(
        (jobResult.data as PolarisJob).title ||
          (jobResult.data as PolarisJob).company_name ||
          'L&D Starmap Report'
      );
      ...
      const hasContent = (tab: ReportType) =>
        !!getJobReportContent(jobResult.data as PolarisJob, tab);
      if (hasContent('final')) {
        setActiveTab('final');
      } else {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[rgb(var(--bg))]">
        <div className="border-primary-400/20 border-t-primary-400 h-16 w-16 animate-spin rounded-full border-4"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[rgb(var(--bg))] text-white">
      {/* Header */}
      <div className="relative overflow-hidden border-b border-white/10 bg-[rgb(var(--bg))]/60 backdrop-blur-xl">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                {editTitleMode ? (
                  <input
                    ...
                    onBlur={saveTitle}
                    className="focus:border-primary-400 border-b border-white/20 bg-transparent text-3xl font-bold text-white outline-none"
                    autoFocus
                  />
                ) : (
              ...
              <div className="mt-2 flex items-center gap-4">
                <span className="text-sm text-white/60">ID: {job.id.slice(0, 8)}...</span>
                <span
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <div className="text-2xl font-bold text-white">{job.edits_remaining}</div>
                <div className="text-xs tracking-wider text-white/60 uppercase">Edits Left</div>
              </div>
              {(job.status === 'draft' || job.status === 'processing') && (
                <button
                  onClick={() => navigate(`/polaris/job/${job.id}/resume`)}
                  className="transform rounded-lg bg-white/10 px-6 py-3 font-medium transition-all hover:scale-105 hover:bg-white/20"
                >
                  Continue Generation
                </button>
              )}
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div
          className={`grid grid-cols-1 ${sidebarCollapsed ? 'lg:grid-cols-1' : 'lg:grid-cols-3'} gap-8`}
        >
          {/* Left Sidebar - Metadata & Activity */}
          <div
            className={`space-y-6 transition-all ${sidebarCollapsed ? 'hidden lg:hidden' : 'lg:col-span-1'}`}
          >
            {/* Sidebar Toggle */}
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
              ...
              >
                ...
                <svg
                  className={`h-5 w-5 transition-transform ${expandedSections.has('overview') ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {expandedSections.has('overview') && (
                <div className="space-y-4 border-t border-white/10 px-6 pb-6">
                  {job.company_name && (
                    <div>
                      <p className="mb-1 text-xs tracking-wider text-white/60 uppercase">Company</p>
                      <p className="font-medium text-white/90">{job.company_name}</p>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <p className="flex items-center gap-2 text-white/90">
                      <span className="text-lg">
                        {STAGE_ICONS[job.current_stage as keyof typeof STAGE_ICONS]}
                      </span>
                      {job.current_stage.replace(/_/g, ' ')}
                    </p>
                  </div>
                  <div>
                    <p className="mb-1 text-xs tracking-wider text-white/60 uppercase">Progress</p>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {Object.keys(STAGE_ICONS)
                        .slice(0, -1)
                        .map((stage) => (
                          <span
                            key={stage}
                  ...
                  <div>
                    <p className="mb-1 text-xs tracking-wider text-white/60 uppercase">Created</p>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/90">{formatDate(job.last_activity_at)}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {(job.greeting_research_status ||
              ...
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
                ...
                >
                  ...
                  <svg
                    className={`h-5 w-5 transition-transform ${expandedSections.has('research') ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {expandedSections.has('research') && (
                  <div className="space-y-3 border-t border-white/10 px-6 pb-6">
                    {job.greeting_research_status && (
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-white/80">Greeting Research</span>
                        <span
                          className={`rounded px-2 py-1 text-xs font-medium ${
                            job.greeting_research_status === 'completed'
                              ? 'bg-green-500/20 text-green-300'
                              : job.greeting_research_status === 'running'
                                ? 'bg-yellow-500/20 text-yellow-300'
                                : job.greeting_research_status === 'failed'
                                  ? 'bg-red-500/20 text-red-300'
                    ...
                    {job.org_research_status && (
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`rounded px-2 py-1 text-xs font-medium ${
                            job.org_research_status === 'completed'
                              ? 'bg-green-500/20 text-green-300'
                              : job.org_research_status === 'running'
                                ? 'bg-yellow-500/20 text-yellow-300'
                                : job.org_research_status === 'failed'
                                  ? 'bg-red-500/20 text-red-300'
                                  : 'bg-gray-500/20 text-gray-300'
                          }`}
                        >
                    ...
                    {job.requirement_research_status && (
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`rounded px-2 py-1 text-xs font-medium ${
                            job.requirement_research_status === 'completed'
                              ? 'bg-green-500/20 text-green-300'
                              : job.requirement_research_status === 'running'
                                ? 'bg-yellow-500/20 text-yellow-300'
                                : job.requirement_research_status === 'failed'
                                  ? 'bg-red-500/20 text-red-300'
                                  : 'bg-gray-500/20 text-gray-300'
                          }`}
                        >
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
              ...
              >
                ...
                <svg
                  className={`h-5 w-5 transition-transform ${expandedSections.has('data') ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {expandedSections.has('data') && (
                <div className="max-h-96 space-y-4 overflow-y-auto border-t border-white/10 px-6 pb-6">
                  <div>
            ...
            {activity.length > 0 && (
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
                ...
                >
                  ...
                  <svg
                    className={`h-5 w-5 transition-transform ${expandedSections.has('activity') ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {expandedSections.has('activity') && (
                  <div className="max-h-96 overflow-y-auto border-t border-white/10 px-6 pb-6">
                    <div className="relative">
                      ...
                      {activity.map((item, index) => (
                        <div key={item.id} className="relative flex items-start gap-4 py-3">
                          <div className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-xs font-bold">
          ...
          {/* Main Content Area */}
          <div
            className={`${sidebarCollapsed ? 'lg:col-span-1' : 'lg:col-span-2'} space-y-6 transition-all`}
          >
            {sidebarCollapsed && (
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
                ...
                {REPORT_TABS.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!hasContent}
                      className={`flex items-center gap-2 border-b-2 px-6 py-4 text-sm font-medium whitespace-nowrap transition-all ${
                        activeTab === tab.id
                    ...
                    >
                      ...
                      <span>{tab.label}</span>
                      {edited && (
                        <span className="ml-2 rounded-full bg-yellow-500/20 px-2 py-0.5 text-xs font-medium text-yellow-300">
                          EDITED
                        </span>
                      )}
                      {editCount > 0 && (
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 z-10 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/85 transition-all hover:bg-white/10 hover:text-white"
                >
                  <svg
                    className="h-5 w-5"
                    viewBox="0 0 24 24"
                  ...
                  {editMode ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={cancelEdit}
                          className="btn-ghost rounded-lg px-5 py-2"
                          disabled={saving}
                        >
                          Cancel
                        </button>
                        <button
                          onClick={saveEdit}
                          className="btn-primary rounded-lg px-6 py-2 disabled:opacity-50"
                          disabled={saving}
                        >
                      ...
                      <div className="mb-8">
                        <div className="mb-2 flex items-center gap-3">
                          <span className="text-3xl">
                            {REPORT_TABS.find((t) => t.id === activeTab)?.icon}
                          </span>
                        ...
                        {job && isReportEdited(job, activeTab) && (
                          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-yellow-500/20 px-3 py-1 text-sm text-yellow-300">
                            <svg
                              className="h-4 w-4"
                              fill="none"
                              stroke="currentColor"
                    ...
                    {getEditHistory(activeTab).map((edit) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-white/60">{formatDate(edit.created_at)}</p>
                          {edit.ai_prompt && (
                      ...
                      >
                        <div>
                          <p className="text-sm font-medium text-white">
                            ...
                            {version.is_current && (
                              <span className="ml-2 rounded bg-green-500/20 px-2 py-0.5 text-xs text-green-300">
                                CURRENT
                              </span>
                            )}
                          </p>
      ...
      {infoOpen && (
        <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
          <div
            className="absolute inset-0 bg-[rgb(var(--bg))]/70 backdrop-blur-sm"
            ...
            <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[rgb(var(--bg))] shadow-2xl">
              ...
              <div className="px-6 py-4">
                <dl className="space-y-4">
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Report Type</dt>
                  ...
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Status</dt>
                    <dd className="text-sm font-medium text-white">
                      {job && isReportEdited(job, activeTab) ? '✏️ Edited' : '📄 Original'}
                    </dd>
                  </div>
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Versions</dt>
                    <dd className="text-sm font-medium text-white">
                      {getReportVersions(activeTab).length} version
                      {getReportVersions(activeTab).length !== 1 ? 's' : ''}
                    </dd>
                  </div>
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Word Count</dt>
                    <dd className="text-sm font-medium text-white">
                      {getActiveContent()
                        .split(/\s+/)
                        .filter((w) => w.length > 0)
                  ...
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Character Count</dt>
                    <dd className="text-sm font-medium text-white">
                      {getActiveContent().length.toLocaleString()} characters
                    </dd>
                  </div>
                  <div>
                    <dt className="mb-1 text-sm text-white/60">Job ID</dt>
                    <dd className="font-mono text-sm font-medium text-white">{job.id}</dd>
                  </div>
                </dl>
              </div>
              <div className="flex justify-end border-t border-white/10 px-6 py-4">
                <button
                  onClick={() => setInfoOpen(false)}
                  className="rounded-lg bg-white/10 px-6 py-2 font-medium transition-all hover:bg-white/20"
                >
                  Close
                </button>
              </div>
