    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^#{2,}\s+/.test(line)) {
        const header = line
          ...
          .toLowerCase();
        // Core sections
        if (/^executive\s+summary/.test(header)) {
          currentSection = 'summary';
          currentSubSection = '';
        } else if (/^recommended\s+solution/.test(header)) {
          currentSection = 'solution';
          currentSubSection = '';
        } else if (/^learner\s+analysis/.test(header)) {
          currentSection = 'learner_analysis';
          currentSubSection = '';
        } else if (/^technology\s+(&|and)?\s+talent(\s+analysis)?/.test(header)) {
          currentSection = 'technology_talent';
          currentSubSection = '';
        } else if (/^delivery\s+plan/.test(header)) {
          currentSection = 'delivery_plan';
          currentSubSection = '';
        } else if (/^measurement(\s+strategy)?/.test(header)) {
          currentSection = 'measurement';
          currentSubSection = '';
        } else if (/^budget/.test(header)) {
          currentSection = 'budget';
          currentSubSection = '';
        } else if (/^risk/.test(header)) {
          currentSection = 'risks';
          currentSubSection = '';
        } else if (/^next\s+steps|^recommendations\s+for\s+success/.test(header)) {
          currentSection = 'next_steps';
          currentSubSection = '';
        }
        // Final-report specific synonyms mapped to structured fields
        else if (/^audience\s+analysis|^target\s+audience/.test(header)) {
          currentSection = 'solution';
          currentSubSection = 'target_audiences';
        } else if (/^delivery\s+approach/.test(header)) {
          currentSection = 'solution';
          currentSubSection = 'delivery_modalities';
        } else if (/^competency\s+development\s+focus/.test(header)) {
          currentSection = 'solution';
          currentSubSection = 'key_competencies';
        } else if (/^proposed\s+curriculum\s+structure/.test(header)) {
          currentSection = 'solution';
          currentSubSection = 'content_outline';
        } else if (/^technology\s+considerations/.test(header)) {
          currentSection = 'technology_talent';
          currentSubSection = 'tech_current';
        } else if (/^accessibility\s+implementation/.test(header)) {
          currentSection = 'solution';
          currentSubSection = 'accessibility';
        } else if (/^implementation\s+roadmap/.test(header)) {
          currentSection = 'delivery_plan';
          currentSubSection = 'phases';
        }
        continue;
      }
      ...
      if (currentSection === 'solution') {
        if (line.startsWith('### Delivery Modalities')) currentSubSection = 'delivery_modalities';
        else if (/^\*\*Target Audience(s)?:\*\*/i.test(line))
          currentSubSection = 'target_audiences';
        else if (/^\*\*Key Competenc(y|ies):\*\*/i.test(line))
          currentSubSection = 'key_competencies';
          ...
          if (currentSubSection === 'delivery_modalities') {
            ...
            report.solution.delivery_modalities.push({
              modality: modalityClean,
              reason,
              priority: report.solution.delivery_modalities.length + 1,
            });
          } else if (currentSubSection === 'target_audiences')
            report.solution.target_audiences.push(item);
          else if (currentSubSection === 'key_competencies')
            report.solution.key_competencies.push(item);
          else if (currentSubSection === 'content_outline')
            report.solution.content_outline.push(item);
      ...
      if (currentSection === 'delivery_plan') {
        if (line.startsWith('### Timeline')) currentSubSection = 'timeline';
          ...
          if (name)
            report.delivery_plan.phases.push({
              name,
              duration_weeks: 0,
              goals: [],
              activities: [],
      ...
      if (
        currentSection === 'measurement' ||
        (currentSection === '' && /success\s+metrics/i.test(line))
      ) {
        // Support both legacy bold labels and newer subheadings
        if (
          line.startsWith('**Success Metrics:**') ||
          /^###\s+Success Metrics/i.test(line) ||
          /^\*\*Measurement\*\*/.test(line)
        ) {
          currentSection = 'measurement';
          currentSubSection = 'success_metrics';
        } else if (
          line.startsWith('**Assessment Strategy:**') ||
          /^###\s+Assessment Strategy/i.test(line)
        ) {
          ...
          if (currentSubSection === 'success_metrics') {
            ...
            currentSubSection === 'assessment_strategy' ||
            /measurement\s+strategy/i.test(rawLine)
          ) {
      ...
      if (currentSection === 'technology_talent') {
        ...
        if (/^###\s+Technology/.test(line)) {
          ...
          line.startsWith('**Current Stack:**') ||
          /^\*\*Available Technologies?:\*\*/i.test(line)
        ) {
          currentSubSection = 'tech_current';
          ...
          currentSubSection.startsWith('tech')
        ) {
          ...
          line.startsWith('**Recommendations:**') &&
          currentSubSection.startsWith('talent')
        ) {
      ...
      if (currentSection === 'learner_analysis') {
        if (/^###\s+Learner Profile/.test(line)) {
          ...
          if (match && report.learner_analysis.profiles.length > 0) {
            const lastProfile =
              report.learner_analysis.profiles[report.learner_analysis.profiles.length - 1];
            const key = match[1].toLowerCase();
            const value = match[2];
            if (key.includes('segment')) lastProfile.segment = value;
            else if (key.includes('role'))
              lastProfile.roles = value.split(',').map((r) => r.trim());
            else if (key.includes('context')) lastProfile.context = value;
            else if (key.includes('motivator'))
              lastProfile.motivators = value.split(',').map((m) => m.trim());
            else if (key.includes('constraint'))
              lastProfile.constraints = value.split(',').map((c) => c.trim());
          }
        } else if (line.startsWith('- ')) {
          const item = line.substring(2).trim();
...
export function extractInThisReportInfo(markdown: string): InThisReportInfo | null {
  ...
  if (report.delivery_plan.phases.length > 0 || report.delivery_plan.timeline.length > 0)
    sections.push('Delivery Plan');
