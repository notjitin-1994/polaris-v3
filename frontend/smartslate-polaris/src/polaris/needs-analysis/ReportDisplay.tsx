...
const PriorityBadge = memo(({ level }: { level: 'high' | 'medium' | 'low' }) => {
  ...
  return (
    <span className={`rounded-full border px-2 py-1 text-xs font-medium ${getBadgeStyles()}`}>
      {getLabel()}
    </span>
  );
});
...
const Icon = memo(({ name, className = 'w-4 h-4' }: { name: IconName; className?: string }) => {
  const iconMap = {
    summary: (
      <svg
        className={className}
    ...
    solution: (
      ...
      >
        <path d="M12 2v4" />
        <path d="M5.22 5.22 8 8" />
        <path d="M2 12h4" />
        <path d="M5.22 18.78 8 16" />
        <path d="M12 22v-4" />
        <path d="M18.78 18.78 16 16" />
        <path d="M22 12h-4" />
        <path d="M18.78 5.22 16 8" />
    ...
    assessment: (
      ...
      >
        ...
        <path d="M21 12c-1 9-4 9-9 9s-8 0-9-9" />
        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 2.88L7 14.14 2 9.27l6.91-1.01L12 2z" />
      </svg>
    ),
    target: (
      <svg
  ...
  ({
    ...
    const renderInline = useCallback((text: string) => {
      ...
      return parts.map((part, idx) =>
        idx % 2 === 1 ? (
          <strong key={idx} className="text-white/90">
            {part}
          </strong>
        ) : (
        ...
        for (const raw of lines) {
          ...
          if (/^\*\*.+\*\*$/.test(line)) {
            ...
            nodes.push(
              <h4
                key={`h4-${nodes.length}`}
                className="mt-4 mb-2 text-sm font-semibold text-white/90"
              >
          ...
          flushList();
          nodes.push(
            <p key={`p-${nodes.length}`} className="text-sm leading-relaxed text-white/80">
              {renderInline(line)}
            </p>
          );
        }
    ...
    const normalizedMarkdown = useMemo(
      () => convertNaJsonStringToMarkdown(reportMarkdown) || reportMarkdown,
      [reportMarkdown]
    );
      ...
      () =>
        (showAllRisks ? report?.risks || [] : (report?.risks || []).slice(0, 3)).map((r, i) => {
          ...
          return (
            <div
              key={i}
              className={`rounded-lg border p-3 ${color} transition-opacity hover:opacity-90`}
            >
              <div className="mb-2 flex items-start justify-between">
                <PriorityBadge level={sev} />
              </div>
              <div className="mb-2 text-sm font-medium text-white/90">{r.risk}</div>
    ...
    const timelineItems = useMemo(
      () =>
        report?.delivery_plan?.timeline?.map((t, i) => {
          ...
          return (
            <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
              <div className="mb-2 flex items-start justify-between">
                <span className="text-sm font-medium text-white/90">{t.label}</span>
                {duration > 0 && (
                  <span className="text-xs font-medium text-amber-400">{duration}w</span>
                )}
              </div>
        ...
        report?.solution?.delivery_modalities?.map((s: any, i: number) => (
          <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
            <div className="mb-2 text-sm font-medium text-white/90">
              {s?.modality || 'Modality'}{' '}
              {(s?.priority ?? i + 1) ? `(P${s?.priority ?? i + 1})` : ''}
            </div>
            <div className="text-xs leading-relaxed text-white/70">{s?.reason || ''}</div>
          </div>
        )) || [],
    ...
    const deliveryPlanItems = useMemo(
      () =>
        report?.delivery_plan?.phases?.map(
          (
            s: { name: string; duration_weeks: number; goals: string[]; activities: string[] },
            i: number
          ) => (
            <div key={i} className="flex items-start gap-3">
              <div className="bg-secondary-500/20 text-secondary-400 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold">
                {i + 1}
              </div>
              <div className="flex-1">
                <div className="mb-1 text-sm font-medium text-white/90">{s.name}</div>
                <div className="text-xs leading-relaxed text-white/70">
                  <div className="mb-2">
                    <span className="font-medium text-white/80">Duration:</span> {s.duration_weeks}{' '}
                    weeks
                  </div>
                  <div className="mb-2">
                  ...
                  <div>
                    <span className="font-medium text-white/80">Activities:</span>{' '}
                    {s.activities.join(', ')}
                  </div>
                </div>
              </div>
        ...
        report?.measurement?.success_metrics?.map((m: any, i: number) => (
          <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm font-medium text-white/90">Metric {i + 1}</span>
            </div>
            {typeof m === 'string' ? (
              <div className="text-xs leading-relaxed text-white/70">{m}</div>
            ) : (
              <div className="text-xs leading-relaxed text-white/70">
    ...
    const audienceItems = useMemo(
      () =>
        report?.solution?.target_audiences?.map((a: string, i: number) => (
          <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
            <div className="mb-2 text-sm font-medium text-white/90">Audience {i + 1}</div>
            <div className="text-xs leading-relaxed text-white/70">{a}</div>
          </div>
        )) || [],
    ...
    const technologyItems = useMemo(
      () =>
        report?.technology_talent?.technology?.current_stack?.map((t: string, i: number) => (
          <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
            <div className="mb-2 text-sm font-medium text-white/90">Technology {i + 1}</div>
            <div className="text-xs leading-relaxed text-white/70">{t}</div>
          </div>
        )) || [],
    ...
    const assessmentItems = useMemo(
      () =>
        report?.measurement?.assessment_strategy?.map((a: string, i: number) => (
          <div key={i} className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8">
            <div className="mb-2 text-sm font-medium text-white/90">Assessment {i + 1}</div>
            <div className="text-xs leading-relaxed text-white/70">{a}</div>
          </div>
        )) || [],
            ...
            <div className="flex items-center gap-3">
              {isEditingTitle ? (
                <input
                  ...
                  className="focus:ring-primary-500/50 focus:border-primary-500/80 flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-2xl font-bold text-white/90 focus:ring-2 focus:outline-none"
                  autoFocus
                />
              ) : (
              ...
              {!isEditingTitle && (
                <button
                  ...
                  className="rounded-lg p-2 text-white/60 transition-colors hover:bg-white/10 hover:text-white/80"
                  title="Edit title"
                >
                  <svg
                    className="h-4 w-4"
            ...
            <div className="flex items-center justify-between">
              ...
              {summaryId && (
                <div className="relative">
                  <button
                    onClick={handleShare}
                    className={`touch-manipulation rounded-full p-2 text-white/60 transition-all hover:bg-white/10 hover:text-white`}
                    title="Copy link"
                  >
                    <svg
                      className="h-5 w-5"
                  ...
                  {showCopySuccess && (
                    <div className="bg-primary-400/20 border-primary-400/30 text-primary-400 animate-in fade-in slide-in-from-top-1 absolute top-full right-0 z-50 mt-2 rounded-lg border p-3 text-sm whitespace-nowrap">
    ...
    const hasStructuredContent = useMemo(() => {
      ...
      const hasSolution =
        (report.solution?.delivery_modalities?.length || 0) > 0 ||
              ...
              <div key={idx} className="glass-card p-5">
                <div className="mb-3 flex items-center gap-2">
                  <div className="rounded bg-white/10 p-1">
                    <Icon name={getIconForSection(s.title)} className="h-4 w-4 text-white/70" />
                  </div>
                  <h3 className="text-lg font-semibold text-white/90">{s.title}</h3>
                </div>
                <div className="space-y-2">{renderLines(s.lines)}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            {report.summary && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="bg-primary-500/20 rounded p-1">
                    <Icon name="summary" className="text-primary-400 h-4 w-4" />
                  </div>
                ...
                <div className="space-y-4 text-sm leading-relaxed text-white/80">
                  ...
                  {report.summary.current_state && report.summary.current_state.length > 0 && (
                    <div>
                      <div className="mb-1 font-medium text-white/80">Current State:</div>
                  ...
                  {report.summary.root_causes && report.summary.root_causes.length > 0 && (
                    <div>
                      <div className="mb-1 font-medium text-white/80">Root Causes:</div>
                      <ul className="list-disc space-y-1 pl-5">
                  ...
                  {report.summary.objectives && report.summary.objectives.length > 0 && (
                    <div>
                      <div className="mb-1 font-medium text-white/80">Objectives:</div>
                      <ul className="list-disc space-y-1 pl-5">
            ...
            {report.solution?.delivery_modalities &&
              report.solution.delivery_modalities.length > 0 && (
                <div className="glass-card p-5">
                  <div className="mb-4 flex items-center gap-2">
                    <div className="bg-secondary-500/20 rounded p-1">
                      <Icon name="solution" className="text-secondary-400 h-4 w-4" />
                    </div>
                    <h3 className="text-lg font-semibold text-white/90">Recommended Solutions</h3>
                  </div>
                  <div className="space-y-3">{solutionItems}</div>
                </div>
              )}
              ...
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-purple-500/20 p-1">
                    <Icon name="toolbox" className="h-4 w-4 text-purple-400" />
                  </div>
            ...
            {report.solution?.content_outline && report.solution.content_outline.length > 0 && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-indigo-500/20 p-1">
                    <Icon name="doc" className="h-4 w-4 text-indigo-400" />
                  </div>
            ...
            {report.learner_analysis?.profiles && report.learner_analysis.profiles.length > 0 && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="bg-primary-500/20 rounded p-1">
                    <Icon name="audience" className="text-primary-400 h-4 w-4" />
                  </div>
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {report.learner_analysis.profiles.map((p, i) => (
                    <div
                      key={i}
                      className="rounded-lg bg-white/5 p-3 transition-colors hover:bg-white/8"
                    >
                      <div className="mb-1 text-sm font-medium text-white/90">{p.segment}</div>
                      <div className="text-xs leading-relaxed text-white/70">
                        {p.roles?.length ? (
                          <div>
                            <span className="font-medium text-white/80">Roles:</span>{' '}
                            {p.roles.join(', ')}
                          </div>
                        ) : null}
                        {p.context ? (
                          <div className="mt-1">
                            <span className="font-medium text-white/80">Context:</span> {p.context}
                          </div>
                        ) : null}
                        {p.motivators?.length ? (
                          <div className="mt-1">
                            <span className="font-medium text-white/80">Motivators:</span>{' '}
                            {p.motivators.join(', ')}
                          </div>
                        ) : null}
                ...
                {report.learner_analysis.readiness_risks?.length ? (
                  <div className="mt-4">
                    <div className="mb-1 font-medium text-white/80">Readiness Risks</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
              ...
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-emerald-500/20 p-1">
                    <Icon name="delivery" className="h-4 w-4 text-emerald-400" />
                  </div>
                  <h3 className="text-lg font-semibold text-white/90">Delivery Plan</h3>
                </div>
                <div className="space-y-4">{deliveryPlanItems}</div>
              </div>
            )}
                ...
                <div className="glass-card p-5">
                  <div className="mb-4 flex items-center gap-2">
                    <div className="rounded bg-blue-500/20 p-1">
                      <Icon name="metrics" className="h-4 w-4 text-blue-400" />
                    </div>
                    <h3 className="text-lg font-semibold text-white/90">Success Metrics</h3>
                  </div>
                  <div className="space-y-3">{metricsItems}</div>
                </div>
              )}
            ...
            {(report.measurement?.data_sources?.length ||
              ...
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-orange-500/20 p-1">
                    <Icon name="assessment" className="h-4 w-4 text-orange-400" />
                  </div>
                  <h3 className="text-lg font-semibold text-white/90">Measurement Details</h3>
                </div>
                {report.measurement?.data_sources?.length ? (
                  <div className="mb-3">
                    <div className="mb-1 font-medium text-white/80">Data Sources</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                      {report.measurement.data_sources.map((d, i) => (
                        <li key={i}>{d}</li>
                      ))}
                    </ul>
                  </div>
                ...
                report.measurement?.learning_analytics?.reporting_cadence ? (
                  <div>
                    <div className="mb-1 font-medium text-white/80">Learning Analytics</div>
                    {report.measurement.learning_analytics.levels?.length ? (
                      <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                        {report.measurement.learning_analytics.levels.map((l, i) => (
                          <li key={i}>{l}</li>
                        ))}
                      </ul>
                    ) : null}
            ...
            {report.budget && (report.budget.items?.length || report.budget.notes) && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-cyan-500/20 p-1">
                    <Icon name="database" className="h-4 w-4 text-cyan-400" />
                  </div>
                  <h3 className="text-lg font-semibold text-white/90">Budget</h3>
                </div>
                {report.budget.notes ? (
                  <div className="mb-3 text-sm leading-relaxed text-white/80">
                    {report.budget.notes}
                  </div>
                ) : null}
                {report.budget.items?.length ? (
                  <div className="space-y-2">
                    {report.budget.items.map((b, i) => (
                      ...
                      >
                        <div className="text-sm font-medium text-white/90">{b.item}</div>
                        <div className="text-xs text-white/70">
                          {report.budget.currency || 'USD'} {b.low} – {b.high}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {report.next_steps && report.next_steps.length > 0 && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-emerald-500/20 p-1">
                    <Icon name="steps" className="h-4 w-4 text-emerald-400" />
                  </div>
            ...
            {report.delivery_plan?.timeline && report.delivery_plan.timeline.length > 0 && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <Icon name="timeline" className="h-4 w-4 text-amber-400" />
            ...
            {report.risks && report.risks.length > 0 && (
              <div className="glass-card border-l-4 border-l-red-400 p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-red-500/20 p-1">
                    <Icon name="risks" className="h-4 w-4 text-red-400" />
                  </div>
          ...
          <div className="space-y-6">
            ...
            {report.solution?.target_audiences && report.solution.target_audiences.length > 0 && (
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-purple-500/20 p-1">
                    <Icon name="audience" className="h-4 w-4 text-purple-400" />
                  </div>
                  <h4 className="text-sm font-semibold text-white/90">Target Audience</h4>
                </div>
                <div className="space-y-3">{audienceItems}</div>
              </div>
            )}
            ...
            {(report.solution?.accessibility_and_inclusion?.standards?.length ||
              ...
              <div className="glass-card p-5">
                ...
                {report.solution.accessibility_and_inclusion.standards?.length ? (
                  <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                    {report.solution.accessibility_and_inclusion.standards.map((s, i) => (
                      <li key={i}>{s}</li>
                    ))}
                  </ul>
                ) : null}
            ...
            {(report.technology_talent?.technology?.current_stack?.length ||
              ...
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-cyan-500/20 p-1">
                    <Icon name="tech" className="h-4 w-4 text-cyan-400" />
                  </div>
                  <h4 className="text-sm font-semibold text-white/90">Technology & Data</h4>
                </div>
                {report.technology_talent.technology.current_stack?.length ? (
                  <div className="mb-3">
                    <div className="mb-1 font-medium text-white/80">Current Stack</div>
                    <div className="space-y-3">{technologyItems}</div>
                  </div>
                ) : null}
                {report.technology_talent.technology.gaps?.length ? (
                  <div className="mb-3">
                    <div className="mb-1 font-medium text-white/80">Gaps</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                      {report.technology_talent.technology.gaps.map((g, i) => (
                        <li key={i}>{g}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {report.technology_talent.technology.recommendations?.length ? (
                  <div className="mb-3">
                    <div className="mb-1 font-medium text-white/80">Recommendations</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                ...
                report.technology_talent.technology.data_plan?.integrations?.length ? (
                  <div>
                    <div className="mb-1 font-medium text-white/80">Data Plan</div>
                    {report.technology_talent.technology.data_plan.standards?.length ? (
                      <div className="mb-1 text-xs text-white/70">
                        Standards:{' '}
                        {report.technology_talent.technology.data_plan.standards.join(', ')}
                      </div>
                    ) : null}
                    {report.technology_talent.technology.data_plan.integrations?.length ? (
                      <div className="text-xs text-white/70">
                        Integrations:{' '}
                        {report.technology_talent.technology.data_plan.integrations.join(', ')}
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            {report.measurement?.assessment_strategy &&
              report.measurement.assessment_strategy.length > 0 && (
                <div className="glass-card p-5">
                  ...
                  <div className="space-y-3">{assessmentItems}</div>
                </div>
              )}
            ...
            {(report.technology_talent?.talent?.available_roles?.length ||
              ...
              <div className="glass-card p-5">
                <div className="mb-4 flex items-center gap-2">
                  <div className="rounded bg-emerald-500/20 p-1">
                    <Icon name="coach" className="h-4 w-4 text-emerald-400" />
                  </div>
                  <h4 className="text-sm font-semibold text-white/90">Talent Overview</h4>
                </div>
                {report.technology_talent.talent.available_roles?.length ? (
                  <div className="mb-2 text-sm text-white/80">
                    <span className="font-medium text-white/80">Available Roles:</span>{' '}
                    {report.technology_talent.talent.available_roles.join(', ')}
                  </div>
                ) : null}
                {report.technology_talent.talent.gaps?.length ? (
                  <div className="mb-2">
                    <div className="mb-1 font-medium text-white/80">Gaps</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                      {report.technology_talent.talent.gaps.map((g, i) => (
                        <li key={i}>{g}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {report.technology_talent.talent.recommendations?.length ? (
                  <div>
                    <div className="mb-1 font-medium text-white/80">Recommendations</div>
                    <ul className="list-disc space-y-1 pl-5 text-sm text-white/80">
                      {report.technology_talent.talent.recommendations.map((r, i) => (
                        <li key={i}>{r}</li>
                      ))}
                    </ul>
                  </div>
