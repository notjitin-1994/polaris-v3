...
export default function ApiDebug() {
  ...
  const filteredCalls = useMemo(() => {
    return logs.filter((l) => {
      const matchesQ =
        !query || (l.url + ' ' + (l.error || '')).toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-[#0A1628] p-4 text-white">
      <div className="mx-auto max-w-[1400px]">
        <h1 className="mb-2 text-2xl font-semibold">
          API Debug {import.meta.env.DEV ? '(dev)' : '(prod)'}
        </h1>
        <div className="mb-4 text-sm text-white/60">
          Track API calls and runtime errors in one place.
        </div>
        {!import.meta.env.DEV && (
          <div className="mb-4 rounded bg-yellow-500/10 p-3 text-yellow-300">
            This page is intended for development use.
          </div>
        )}
        ...
        <div className="mb-3 flex items-center gap-2">
          <button
            ...
            className={`rounded border px-3 py-1.5 text-sm ${tab === 'calls' ? 'border-white/30 bg-white/15' : 'border-white/10 bg-white/5 hover:bg-white/10'}`}
          >
            Calls
          </button>
          <button
            onClick={() => setTab('errors')}
            className={`rounded border px-3 py-1.5 text-sm ${tab === 'errors' ? 'border-white/30 bg-white/15' : 'border-white/10 bg-white/5 hover:bg-white/10'}`}
          >
            Errors
          </button>
          <div className="ml-auto flex gap-2 text-xs text-white/60">
            ...
            <SummaryBadge
              label="HTTP 4xx/5xx"
              value={logs.filter((l) => (l.status ?? 0) >= 400).length}
            />
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          <input
            ...
            placeholder={
              tab === 'calls' ? 'Filter by URL or error' : 'Filter by message, origin, code'
            }
            className="w-80 rounded border border-white/20 bg-white/10 px-3 py-2 outline-none"
          />
          {tab === 'calls' && (
          ...
          {tab === 'errors' && (
            <select
              ...
              className="rounded border border-white/20 bg-white/10 px-3 py-2 text-sm"
            >
          ...
          {tab === 'errors' && (
            <button
              ...
              className="rounded border border-white/20 bg-white/10 px-3 py-2 text-sm hover:bg-white/20"
            >
              Export JSON
            </button>
          )}
        ...
        {tab === 'calls' ? (
                ...
                {filteredCalls.length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-6 text-center text-white/50">
                      No logs yet. Trigger some API calls.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            ...
            <div className="min-h-[360px] rounded border border-white/10 bg-white/5 p-3">
              {!selectedError && (
                <div className="text-sm text-white/50">Select an error to see details.</div>
              )}
              {selectedError && (
                <div className="space-y-3 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="rounded border border-red-400/30 bg-red-500/10 px-2 py-0.5 text-red-300">
                      {selectedError.type}
                    </span>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <Section
                      title="Breadcrumbs"
                      content={(selectedError.breadcrumbs || []).map((b) => ({
                        t: b.timestamp,
                        c: b.category,
                        m: b.message,
                        d: b.data,
    ...
    <>
      <tr className="border-t border-white/10">
        ...
        <td className="p-2 whitespace-nowrap">
          <span className="rounded border border-white/10 bg-white/10 px-2 py-0.5 text-white/80">
            {entry.method}
          </span>
        </td>
        <td className="max-w-[32rem] truncate p-2 text-white/80" title={entry.url}>
          {entry.url}
        </td>
        <td className="p-2 whitespace-nowrap">
          {typeof entry.status === 'number' ? (
            <span
              className={`rounded border px-2 py-0.5 ${entry.ok ? 'border-green-400/30 bg-green-500/10 text-green-300' : 'border-red-400/30 bg-red-500/10 text-red-300'}`}
            >
              {entry.status}
            </span>
          ) : (
        ...
        <td className="max-w-[16rem] truncate p-2 text-red-300" title={entry.error}>
          {entry.error || ''}
        </td>
        <td className="p-2">
          <button
...
function Section({ title, content }: { title: string; content: any }) {
  return (
    <div>
      ...
      <pre className="max-h-60 overflow-auto rounded bg-black/30 p-2 whitespace-pre-wrap text-white/80">
...
function ErrorRow({
  ...
  return (
    <>
      <tr
        className={`border-t border-white/10 ${selectedId === entry.id ? 'bg-white/10' : ''}`}
      ...
      >
        ...
        <td className="max-w-[32rem] truncate p-2 text-white/80" title={entry.message}>
          {entry.message}
        </td>
        ...
        <tr className="border-t border-white/10 bg-white/5">
          <td colSpan={7} className="p-3">
            <div className="grid grid-cols-1 gap-3 text-xs md:grid-cols-2">
              ...
              <Section
                title="Breadcrumbs"
                content={(entry.breadcrumbs || []).map((b) => ({
                  t: b.timestamp,
                  c: b.category,
                  m: b.message,
                  d: b.data,
...
async function generateApiTests() {
  ...
  try {
    await fetch('http://localhost:1/ping');
  } catch {}
}
