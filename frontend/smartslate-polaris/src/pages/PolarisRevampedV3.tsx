...
import { NA_QUESTIONNAIRE_PROMPT } from '@/polaris/needs-analysis/prompts';
...
import { StepIndicator, WizardContainer, ActionButtons, ProgressBar } from '@/components';
...
export default function PolarisRevampedV3() {
  ...
  async function generateReport() {
    ...
    try {
      ...
      const prompt = buildFastNAReportPrompt(expVal, allData);
      ...
      setActive('report');
      setGenerationProgress(
        'Your starmap is being generated. You can safely navigate away and return later.'
      );
    } catch (err) {
  ...
  function getLoadingMessage(
    status: string,
    progress: number
  ): { title: string; message: string; phase: string } {
    switch (status) {
      case 'queued':
        return {
          title: 'Your Starmap is Queued',
          message:
            "We're preparing to generate your personalized L&D starmap. This typically takes 1-2 minutes.",
          phase: 'queued',
        };
      case 'running':
        if (progress < 20) {
          return {
            ...
            message:
              'Processing your organization details and project requirements to create targeted questions.',
            phase: 'analyzing',
          };
        } else if (progress < 40) {
          return {
            ...
            message:
              'Gathering industry insights and L&D methodologies relevant to your situation.',
            phase: 'researching',
          };
        } else if (progress < 60) {
          return {
            ...
            message:
              'Developing personalized recommendations and action plans based on your unique context.',
            phase: 'crafting',
          };
        } else if (progress < 80) {
          return {
            ...
            message:
              'Polishing the details and ensuring all recommendations are actionable and well-structured.',
            phase: 'finalizing',
          };
        } else {
          return {
            ...
            message:
              'Adding final touches and preparing your comprehensive L&D starmap for delivery.',
            phase: 'completing',
          };
        }
      case 'processing':
      ...
      default:
        return {
          ...
          message:
            "You can safely navigate away and return later. We'll save your report when it's ready.",
          phase: 'generating',
        };
    }
  }
    ...
    switch (active) {
      ...
      case 'dynamic':
        return (
          ...
          dynamicQuestions.every((f) => isFieldAnswered(f, dynamicAnswers[f.id]))
        );
      default:
        return true;
    }
  ...
  return (
    <div className="min-h-screen bg-[rgb(var(--bg))]">
      ...
      <div className="mx-auto max-w-6xl px-4 pb-12 sm:px-6 lg:px-8">
        {error && (
          <div className="mb-6 rounded-lg border border-red-500/30 bg-red-500/20 p-4 text-red-100">
            {error}
        ...
        {generating && !asyncJobId ? (
          ...
          >
            <div className="py-12 text-center">
              <div className="border-primary-500 mx-auto mb-4 h-16 w-16 animate-spin rounded-full border-b-2"></div>
              <p className="mb-4 text-lg text-white/80">
                {generationProgress || 'Initializing AI analysis...'}
              </p>
              <div className="bg-primary-500/20 border-primary-500/30 inline-flex items-center gap-2 rounded-full border px-4 py-2">
                <div className="bg-primary-400 h-2 w-2 animate-pulse rounded-full"></div>
                <span className="text-primary-300 text-sm font-medium">Preparing</span>
              </div>
            </div>
          </WizardContainer>
        ) : active === 'report' &&
          ...
          >
            <div className="py-12 text-center">
              ...
              <div className="mb-6">
                <div className="bg-primary-500/20 border-primary-500/30 inline-flex items-center gap-2 rounded-full border px-4 py-2">
                  <div className="bg-primary-400 h-2 w-2 animate-pulse rounded-full"></div>
                  <span className="text-primary-300 text-sm font-medium capitalize">
              ...
              <p className="mb-8 text-white/70">
                This typically takes 1-2 minutes. Feel free to browse other sections or come back
                later.
              </p>
              <button onClick={() => navigate('/starmaps')} className="btn-primary px-6 py-2">
                View All Starmaps
              </button>
            </div>
          </WizardContainer>
          ...
          >
                ...
                {EXPERIENCE_LEVELS.map((field) => (
                  <div key={field.id}>
                    <RenderField
                      ...
                      onChange={(id, value) =>
                        setExperienceAnswer((prev) => ({ ...prev, [id]: value }))
                      }
                    />
                  </div>
                ))}
                ...
                {STAGE1_REQUESTER_FIELDS.map((field) => (
                  <div key={field.id}>
                    <RenderField
                      ...
                      onChange={(id, value) =>
                        setStage1Answers((prev) => ({ ...prev, [id]: value }))
                      }
                    />
                  </div>
                ))}
                ...
                {STAGE2_ORGANIZATION_FIELDS.map((field) => (
                  <div key={field.id} className={field.type === 'textarea' ? 'md:col-span-2' : ''}>
                    <RenderField
                      ...
                      onChange={(id, value) =>
                        setStage2Answers((prev) => ({ ...prev, [id]: value }))
                      }
                    />
                  </div>
                ))}
                ...
                {STAGE3_PROJECT_FIELDS.map((field) => (
                  <div key={field.id} className={field.type === 'textarea' ? 'md:col-span-2' : ''}>
                    <RenderField
                      ...
                      onChange={(id, value) =>
                        setStage3Answers((prev) => ({ ...prev, [id]: value }))
                      }
                    />
                  </div>
                ))}
            ...
            <ActionButtons
              ...
              nextLabel={
                active === 'dynamic'
                  ? 'Generate Starmap'
                  : active === 'stage3' && dynamicQuestions.length === 0
                    ? 'Continue'
                    : 'Next'
              }
