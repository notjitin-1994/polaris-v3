...
const STAGES: {
  ...
  {
    id: 'greeting',
    label: 'Requester & Context',
    icon: '🎯',
    description: 'Tell us about yourself so we can personalize the analysis.',
  ...
  },
  {
    ...
    icon: '🏢',
    description: 'Share org info to calibrate scope, scale, and constraints.',
  ...
  },
  {
    ...
    icon: '🧭',
    description: 'Clarify objectives, timeline, budget, and success criteria.',
  ...
  },
  {
    ...
    icon: '📝',
    description: 'We’ll generate a draft synthesis for your review.',
  },
  {
    id: 'dynamic_questions',
    label: 'Dynamic Questionnaire',
    icon: '🔍',
    description: 'Answer targeted follow-ups to refine recommendations.',
  },
  {
    id: 'final_report',
    label: 'Final Report',
    icon: '📋',
    description: 'Review the final starmap report and share as needed.',
  },
];
...
export default function PolarisJobWizard() {
  ...
  const [prelimStatus, setPrelimStatus] = useState<'idle' | 'processing' | 'completed' | 'failed'>(
    'idle'
  );
  ...
  function restoreJobState(job: PolarisJob) {
    ...
    if (job.current_stage === 'greeting' || job.stages_completed.includes('greeting')) {
      ...
      job.current_stage === 'organization' ||
      job.stages_completed.includes('organization')
    ) {
      setStageAnswers(job.org_data);
    } else if (
      job.current_stage === 'requirements' ||
      job.stages_completed.includes('requirements')
    ) {
  ...
  async function saveProgress(pause: boolean = false) {
    ...
    try {
      ...
      // Update progress
      await updateJobProgress(job.id, currentStage, pause ? 'paused' : 'draft', false);
  ...
  async function handleStageComplete() {
    ...
    try {
      ...
      await updateJobProgress(job.id, currentStage, 'draft', true);
  ...
  async function generateFinalReport() {
    ...
    const dynamicQAReport = dynamicQuestions
      .map((q) => {
  ...
  const currentStage = STAGES[currentStageIndex];
  const completedSteps = (job?.stages_completed || [])
    .map((s) => STAGES.findIndex((st) => st.id === s))
    .filter((i) => i >= 0);
  const stepsForIndicator = STAGES.map((s) => ({
    key: s.id,
    label: s.label,
    icon: s.icon,
    description: s.description,
  }));
  const maxNavigableIndex = Math.max(
    currentStageIndex,
    ...(completedSteps.length ? completedSteps : [0])
  );
  const nextLabel = (() => {
        ...
        <div className="mx-auto max-w-5xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {job && (
                <span className="text-sm text-white/60">Job ID: {job.id.slice(0, 8)}...</span>
              )}
            </div>
          </div>
        </div>
          ...
          {generating ? (
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    {(EXPERIENCE_LEVELS[0] as any).options?.map((option: string, index: number) => (
                      <button
                        ...
                        className="rounded-lg border border-white/10 bg-white/5 p-4 text-left transition-all hover:bg-white/10"
                      >
            ...
            <div>
              <p className="mb-6 text-white/70">
                Based on your initial inputs, please answer these additional questions to refine the
                recommendations.
              </p>
              <div className="space-y-6">
            ...
            <div className="py-12 text-center">
              ...
              {currentStage.id === 'dynamic_questions' && (
                <div className="mt-4">
                  <button onClick={generateDynamicQuestions} className="btn-primary px-6 py-2">
