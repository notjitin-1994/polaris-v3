 ...
 * Get a job by ID
 */
export async function getStarmapJob(
  jobId: string
): Promise<{ data: StarmapJob | null; error: any }> {
  try {
 ...
 */
export async function toggleStarmapPublicStatus(
  jobId: string
 ...
 */
export async function getStarmapPublicStatus(
  jobId: string
): Promise<{ isPublic: boolean | null; error: any }> {
  try {
    const { data, error } = await supabase
...
export async function updateStarmapJobStageData(
  ...
  try {
    ...
    const { error } = await supabase.from('starmap_jobs').update(updateData).eq('id', jobId);
...
export async function checkAsyncJobStatus(
  ...
  try {
    ...
    const response = await fetch(
      `/api/reportJobsDb?job_id=${encodeURIComponent(job.report_job_id)}`
    );
    ...
    await supabase.from('starmap_jobs').update(updateData).eq('id', jobId);
 ...
 * Resume a job
 */
export async function resumeStarmapJob(
  jobId: string
): Promise<{ data: StarmapJob | null; error: any }> {
  try {
...
export async function deleteStarmapJob(jobId: string): Promise<{ error: any }> {
  try {
    const { error } = await supabase.from('starmap_jobs').delete().eq('id', jobId);
...
export async function getJobActivity(
  jobId: string
): Promise<{ data: StarmapJobActivity[]; error: any }> {
  try {
    const { data, error } = await supabase
...
async function logActivity(
  ...
  try {
    ...
    await supabase.from('starmap_job_activity').insert({
      job_id: jobId,
      user_id: user.id,
      action,
      details: details || {},
