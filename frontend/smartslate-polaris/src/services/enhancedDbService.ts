...
export class EnhancedDbService {
   ...
   */
  async query<T>(table: string, options: QueryOptions = {}): Promise<{ data: T[]; error: any }> {
    const {
      cache = true,
  ...
  async batchInsert<T>(
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        const { data, error } = returnData ? await query.select() : await query;
...
export const dbTransaction = (operations: BatchOperation[]) => enhancedDb.transaction(operations);
