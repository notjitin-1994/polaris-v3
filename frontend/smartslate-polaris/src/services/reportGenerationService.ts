...
export class ReportGenerationService {
   ...
   */
  async generateStructuredReport(context: ReportContext): Promise<GeneratedReport> {
...
Return the enhanced report in the same format as the original.`;
  ...
  async generateReportSection(
    ...
    const sectionPrompts = {
      learner_analysis:
        'Generate a comprehensive learner analysis including demographics, preferences, constraints, and readiness',
      technology_assessment:
        'Assess technology requirements, current capabilities, gaps, and recommendations',
      budget_breakdown:
        'Provide detailed budget breakdown with cost categories, ranges, and ROI analysis',
      risk_assessment:
        'Identify and analyze risks with probability, impact, and mitigation strategies',
      implementation_plan:
        'Create a detailed implementation plan with phases, timelines, and success criteria',
    };
...
Return a JSON object with:
{
  ...
  private extractMetadata(report: string, aiResponse: any): GeneratedReport['metadata'] {
    ...
    const gaps =
      report.match(/\b(unknown|unclear|TBD|to be determined|needs clarification)\b/gi) || [];
...
export const validateReport = (report: string) => reportGenService.validateReport(report);
