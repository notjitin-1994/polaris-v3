...
${prompt}`;
    ...
    try {
      ...
      const response = await this.client.post<any>('/perplexity', requestPayload, {
        timeout: 75000,
        retries: 1,
      });
...
Return ONLY the Markdown brief as specified above—no preamble or closing remarks.`;
    const result = await this.researchWithRetry(prompt, {
      model: (env as any).perplexityGreetingModel || 'sonar',
      maxTokens: 1200,
    });
...
Return ONLY the Markdown brief in the format above — no preamble or closing remarks.`;
    const result = await this.researchWithRetry(prompt, {
      model: (env as any).perplexityOrgModel || 'sonar-pro',
      maxTokens: 1800,
    });
    return result.content;
  }
...
Return ONLY the Markdown brief in the format above — no preamble or closing remarks.`;
    const result = await this.researchWithRetry(prompt, {
      model: (env as any).perplexityRequirementModel || 'sonar-reasoning',
      maxTokens: 2000,
    });
    return result.content;
  }
  ...
  async researchWithRetry(
    ...
    return {
      content:
        'Research data temporarily unavailable. Please proceed with the information provided.',
...
export const researchOrganization = (
  data: Parameters<PerplexityService['researchOrganization']>[0]
) => perplexityService.researchOrganization(data);

export const researchRequirements = (
  data: Parameters<PerplexityService['researchRequirements']>[0]
