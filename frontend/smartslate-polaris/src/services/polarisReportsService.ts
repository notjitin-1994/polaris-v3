...
async function createReport(
  ...
  try {
    ...
    const metadata =
      options.metadata ||
      (researchReport ? defaultMetadata({ mode: 'sync' }) : defaultMetadata({ mode: 'pending' }));
...
export async function updateResearchReport(
  ...
  try {
    const tableCandidates: Array<'greeting_reports' | 'org_reports' | 'requirement_reports'> =
      reportType
...
export async function runGreetingResearchSync(
  reportId: string,
  data: {
    name: string;
    role: string;
  ...
  return updateResearchReport(reportId, content, 'greeting', {
    model: env.perplexityGreetingModel || 'sonar',
  });
}

export async function runOrgResearchSync(
  reportId: string,
  data: {
    orgName: string;
    industry: string;
  ...
  return updateResearchReport(reportId, content, 'org', {
    model: env.perplexityOrgModel || 'sonar-pro',
  });
}

export async function runRequirementResearchSync(
  reportId: string,
  data: {
    objectives: string;
    constraints: string;
  ...
  return updateResearchReport(reportId, content, 'requirement', {
    model: env.perplexityRequirementModel || 'sonar-reasoning',
  });
}
...
export async function startResearchJobForReport(args: {
  ...
  try {
    ...
    const response = await apiClient.post<{ job_id: string; status_url: string }>(
      '/reportJobsDb',
      payload,
      { headers }
    );
...
export async function startGreetingResearchAsync(
  reportId: string,
  data: Parameters<typeof buildGreetingPrompt>[0],
  summaryId?: string
) {
  const prompt = buildGreetingPrompt(data);
  return startResearchJobForReport({
    reportType: 'greeting',
    reportId,
    summaryId,
    prompt,
...
export async function startOrgResearchAsync(
  reportId: string,
  data: Parameters<typeof buildOrgPrompt>[0],
  summaryId?: string
) {
...
export async function startRequirementResearchAsync(
  reportId: string,
  data: Parameters<typeof buildRequirementPrompt>[0],
  summaryId?: string
) {
...
export async function applyJobResultToReport(
  jobId: string
): Promise<{ data: any | null; error: any }> {
  try {
  ...
  try {
    ...
    for (const table of tables) {
      const { data, error } = await supabase
        .from(table)
        .select(
          'id, webhook_status, webhook_attempts, webhook_last_attempt, research_status, created_at'
        )
...
export async function getWebhookStatistics(): Promise<{ data: any | null; error: any }> {
  try {
    ...
    for (const table of tables) {
      ...
      const { data, error } = await supabase.from(table).select('webhook_status');
