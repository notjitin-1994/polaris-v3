...
- Balance ideal and practical approaches`,
  ...
  private getContextualEnhancements(context: PromptContext): string {
    ...
    if (context.industry) {
      enhancements.push(
        `INDUSTRY CONTEXT: Consider ${context.industry}-specific regulations, standards, and best practices.`
      );
    }
    ...
    if (context.companySize) {
      ...
      enhancements.push(
        `ORGANIZATION SIZE: Account for ${sizeFactors[factor] || 'organizational scale'}`
      );
    }
    ...
    if (context.budget) {
      enhancements.push(
        `BUDGET CONTEXT: ${context.budget} - Optimize recommendations for cost-effectiveness`
      );
    }
    ...
    if (context.researchData) {
      ...
      if (research.length > 0) {
        enhancements.push(
          `RESEARCH CONTEXT: ${research.join(', ')}. Leverage these insights in your response.`
        );
      }
    }
    ...
    if (context.previousAnswers && Object.keys(context.previousAnswers).length > 0) {
      enhancements.push(
        `BUILD ON PREVIOUS INPUTS: User has already provided detailed information. Avoid redundancy, focus on gaps and clarifications.`
      );
    }
...
Use structured thinking, evidence-based reasoning, and clear recommendations.`,
  ...
  private mapCompanySize(size: string): 'startup' | 'small' | 'medium' | 'large' | 'enterprise' {
    ...
    return (sizeMap[size.toLowerCase()] || 'medium') as
      | 'startup'
      | 'small'
      | 'medium'
      | 'large'
   ...
   */
  generateQuestionnairePrompt(stage: number, context: PromptContext, focusArea?: string): string {
