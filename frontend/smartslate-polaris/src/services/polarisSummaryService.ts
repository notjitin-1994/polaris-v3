...
export async function getUserCreatedCount(): Promise<{ count: number | null; error: any }> {
  ...
  try {
    ...
    if (typeof savedCount === 'number' && savedCount > (value ?? 0)) {
      ...
      await supabase.from('profiles').update({ starmaps_created: savedCount }).eq('id', user.id);
    }
  } catch {}
...
export async function saveSummary(data: {
  ...
  if (!isUnlimited) {
    ...
    if (savedCount !== null && savedCount >= SAVED_LIMIT) {
      return {
        data: null,
        error: new Error(
          `You have reached the saved starmaps limit of ${SAVED_LIMIT}. Delete some or upgrade to save more.`
        ),
      };
    }
  }
  ...
  if (!isUnlimited) {
    ...
    if (createdCount !== null && createdCount >= CREATION_LIMIT) {
      return {
        data: null,
        error: new Error(
          `You have reached the creation limit of ${CREATION_LIMIT} starmaps. Please upgrade to create more.`
        ),
      };
    }
  }
  ...
  const requesterName =
    (data.stage1_answers && (data.stage1_answers as any).requester_name) ||
...
export async function updateSummaryTitle(id: string, _newTitle: string): Promise<{ error: any }> {
  ...
  const requesterName =
    (existing?.stage1_answers && (existing.stage1_answers as any).requester_name) ||
    (user?.user_metadata as any)?.full_name ||
    (user?.email ? (user.email as string).split('@')[0] : null) ||
    'Unknown User';
  const orgName =
    ...
    'Unknown Org';
  const datePart = existing?.created_at
    ? String(existing.created_at).slice(0, 10)
...
export async function toggleReportPublicStatus(
  id: string
): Promise<{ isPublic: boolean | null; error: any }> {
  const supabase = getSupabase();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('toggle_report_public_status', { report_id: id });
...
export async function updateSummaryFinalContent(
  ...
  options: {
    stage2_questions?: any[];
    stage3_questions?: any[];
    stage3_answers_merge?: Record<string, any>;
    dynamic_questionnaire_report?: string | null;
  } = {}
): Promise<{ error: any }> {
  const supabase = getSupabase();
  ...
  if (options.stage3_answers_merge)
    (updatePayload as any).stage3_answers = options.stage3_answers_merge;
  if (options.dynamic_questionnaire_report !== undefined)
    (updatePayload as any).dynamic_questionnaire_report = options.dynamic_questionnaire_report;
...
export async function updateSummaryReports(
  ...
  if (reports.greeting_report !== undefined)
    (payload as any).greeting_report = reports.greeting_report;
  if (reports.org_report !== undefined) (payload as any).org_report = reports.org_report;
  if (reports.requirement_report !== undefined)
    (payload as any).requirement_report = reports.requirement_report;
