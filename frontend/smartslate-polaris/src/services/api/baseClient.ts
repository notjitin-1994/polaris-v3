    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            try {
              reportError(err, {
                type: 'api',
                origin: 'BaseApiClient',
                statusCode: 401,
                code: 'UNAUTHORIZED',
                context: { url, method: config.method || 'GET' },
              });
            } catch {}
            throw err;
          } else if (response.status === 429) {
            const err = new ApiError(errorData.message || 'Rate limit exceeded', 429, {
        ...
        if (error.name === 'AbortError') {
          const err = new TimeoutError(`Request timed out after ${timeout}ms`, {
            timeout,
            url,
