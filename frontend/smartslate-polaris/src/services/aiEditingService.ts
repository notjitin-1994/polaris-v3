    ...
    try {
      ...
      ${
        context.greetingReport
          ? `ORIGINAL RESEARCH - Greeting/Context:
      ${context.greetingReport}
      ...
      `
        ...
        context.orgReport
          ? `ORIGINAL RESEARCH - Organization Analysis:
      ${context.orgReport}
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          const missingSections = requiredSections.filter(
            (section) => !editedContent.includes(section)
          );
          ...
          if (editedLength < originalLength * 0.5) {
            console.error(
              'AI response appears truncated. Original length:',
              originalLength,
              'Edited length:',
              editedLength
            );
