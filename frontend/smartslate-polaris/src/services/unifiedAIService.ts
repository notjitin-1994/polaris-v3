...
export class UnifiedAIService {
  ...
  async call(request: AIRequest): Promise<AIResponse> {
    ...
    console.log(
      `[UnifiedAI] Selected provider: ${provider} for ${request.inputType || 'text'} input`
    );
    ...
    try {
      ...
      switch (provider) {
        case 'perplexity':
          const perplexityResult = await perplexityService.research(request.prompt, {
            model: config.model,
  ...
  async analyzeDocument(
    ...
    const provider = options?.compliance
      ? 'anthropic'
      : options?.contextSize && options.contextSize > 200000
        ? 'google'
        : 'anthropic';
