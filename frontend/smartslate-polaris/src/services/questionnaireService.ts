...
export class QuestionnaireService {
  ...
  async generateDynamicStages(
    ...
    for (let i = 0; i < stageCount; i++) {
      ...
      stagePromises.push(this.generateStage(i + 1, context, focusArea, customPrompt));
    }
...
OUTPUT: Valid JSON only, no other text`;
  ...
  private validateStage(stage: QuestionnaireStage): void {
    ...
    if (stage.questions.length > this.MAX_QUESTIONS_PER_STAGE) {
      console.warn(
        `[Questionnaire] Stage has ${stage.questions.length} questions, trimming to ${this.MAX_QUESTIONS_PER_STAGE}`
      );
  ...
  private optimizeStages(
    ...
    for (const stage of stages) {
      ...
      if (stage.questions.length < this.MIN_QUESTIONS_PER_STAGE) {
        console.warn(
          `[Questionnaire] Stage ${stage.id} has only ${stage.questions.length} questions`
        );
      }
    }
   ...
   */
  applySkipLogic(stages: QuestionnaireStage[], answers: Record<string, any>): QuestionnaireStage[] {
