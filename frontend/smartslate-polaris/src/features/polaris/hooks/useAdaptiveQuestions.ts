...
export function useAdaptiveQuestions(
  ...
  return useMemo(() => {
    ...
    const missing = (id: string) =>
      allAnswers[id] === undefined ||
      allAnswers[id] === null ||
      allAnswers[id] === '' ||
      (Array.isArray(allAnswers[id]) && (allAnswers[id] as unknown[]).length === 0);
    ...
    if (missing('kickoff_window')) {
      questions.push({
        ...
        type: 'calendar_range',
        required: true,
        help: 'Improves timeline and resource planning.',
      });
    }
    if (missing('success_metrics')) {
      questions.push({
        id: 'success_metrics',
        label: 'Key metrics you track',
        type: 'multi_select',
        options: [
          'Completion',
          'Assessment scores',
    ...
    if (experienceLevel && experienceLevel.toLowerCase().includes('novice')) {
      questions.push({
        ...
        label: 'Are stakeholders aligned on objectives?',
        type: 'single_select',
        options: ['Yes', 'Partially', 'No', 'Unsure'],
        help: 'Improves risk and next steps guidance.',
      });
    }
