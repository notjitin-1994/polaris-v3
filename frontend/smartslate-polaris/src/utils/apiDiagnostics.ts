...
export async function runFullDiagnostics(): Promise<{
  ...
  if (perplexityResult.status === 'timeout') {
    recommendations.push(
      'Perplexity API is slow. Consider using faster models or enabling async mode.'
    );
  }

  if (perplexityResult.status === 'auth_error') {
    recommendations.push(
      'Perplexity API key is invalid. Please check your VITE_PERPLEXITY_API_KEY in .env'
    );
  }
