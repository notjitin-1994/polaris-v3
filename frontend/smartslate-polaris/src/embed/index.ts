...
async function renderPicker(options?: PolarisPickerOptions): Promise<Nullable<PolarisSummary>> {
  ...
  const sub = el(
    'div',
    'ssp-sub',
    options?.description || 'Choose one of your existing discovery starmaps to continue'
  );
  ...
  function renderList(items: PolarisSummary[]) {
    ...
    if (!items.length) {
      const empty = el('div', 'ssp-empty');
      empty.innerHTML =
        'No starmaps yet. Create one in <a href="https://app.smartslate.io/" target="_blank" rel="noreferrer noopener" style="text-decoration: underline; color: #9fc5ff;">Smartslate</a>.';
      list.appendChild(empty);
      return;
    }
    for (const s of items) {
      ...
      const t = el(
        'div',
        'ssp-row-title',
        (s.report_title as string) || (s.company_name as string) || 'Untitled Discovery'
      );
...
export async function openStarmapPicker(
  options?: PolarisPickerOptions
): Promise<Nullable<PolarisSummary>> {
