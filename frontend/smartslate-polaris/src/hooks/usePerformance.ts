 ...
 */
export function useExpensiveCalculation<T>(factory: () => T, deps: any[], cacheKey?: string): T {
...
export function useComponentLifecycle(componentName: string) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `${componentName} unmounted after ${totalTime.toFixed(2)}ms (${updateCount.current} updates)`
        );
      }
    };
  }, [componentName]);
    ...
    const updateBundleInfo = () => {
      if ('performance' in window) {
        ...
        const jsFiles = entries.filter(
          (entry) => entry.name.endsWith('.js') || entry.name.endsWith('.mjs')
        );
