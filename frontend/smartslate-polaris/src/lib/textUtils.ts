...
export function markdownToHtml(markdown: string): string {
  ...
  const convertInline = (text: string): string => {
    ...
    t = t.replace(/\[(\d+)\](?!\()/g, (_m, n) =>
      citationMap[n]
        ? `<a href="${citationMap[n]}" target="_blank" rel="noopener noreferrer">[${n}]<\/a>`
        : `[${n}]`
    );
    // Bold/italic/code
    t = t
...
export function htmlToMarkdown(html: string): string {
  ...
  md = md.replace(
    /<font[^>]*color=["']?([^"'>\s]+)["']?[^>]*>/gim,
    (_m, color) => `<span style="color: ${color}">`
  );
    ...
    if (headerMatch) {
      ...
      if (headerCells) {
        const headers = headerCells.map((cell) =>
          cell.replace(/<th[^>]*>([\s\S]*?)<\/th>/gim, '$1').trim()
        );
        ...
        bodyRows.forEach((row) => {
          ...
          if (cells) {
            const cellContents = cells.map((cell) =>
              cell.replace(/<td[^>]*>([\s\S]*?)<\/td>/gim, '$1').trim()
            );
