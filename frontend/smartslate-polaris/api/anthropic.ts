...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const defaultMax =
      Number(process.env.ANTHROPIC_MAX_TOKENS || process.env.VITE_ANTHROPIC_MAX_TOKENS) || 4096;
    ...
    if (!upstream.ok) {
      ...
      res.send(
        text || JSON.stringify({ error: 'Anthropic upstream error', status: upstream.status })
      );
      return;
    }
    res.send(text);
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      // eslint-disable-next-line no-console
      console.error(
        'Anthropic proxy timeout:',
        `Exceeded ${Number(process.env.ANTHROPIC_SERVER_TIMEOUT_MS || 115000)}ms`
      );
      res
        .status(504)
        .json({ error: 'Upstream timeout', detail: 'Anthropic request exceeded server timeout' });
      return;
    }
    // eslint-disable-next-line no-console
