...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const defaultMax =
      Number(process.env.OPENAI_MAX_TOKENS || process.env.VITE_OPENAI_MAX_TOKENS) || 4096;
    ...
    if (!upstream.ok) {
      ...
      try {
        ...
        if (
          (/unsupported_parameter/i.test(msg) || /unsupported_parameter/i.test(code)) &&
          /max_tokens/i.test(msg + code)
        ) {
          shouldFallback = true;
        }
