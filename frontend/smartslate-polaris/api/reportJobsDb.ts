...
const webhookBaseUrl =
  process.env.WEBHOOK_BASE_URL || process.env.VITE_SITE_URL || 'http://localhost:5173';
...
async function buildDynamicQuestionnairePrompt(summaryId: string): Promise<string> {
  ...
  const { data: s } = await supabase
    ...
    .select(
      'company_name, prelim_report, greeting_report, org_report, requirement_report, stage1_answers, stage2_answers, stage3_answers'
    )
    .eq('id', summaryId)
    .single();
...
async function generateDynamicQuestionnaire(
  summaryId: string
): Promise<{ stagesJson: string; stagesObj: any }> {
  ...
  const maxTokens = Math.min(
    parseInt(process.env.ANTHROPIC_MAX_TOKENS || process.env.VITE_ANTHROPIC_MAX_TOKENS || '12000'),
    8192
  );
...
async function callWebhook(
  ...
  try {
    ...
    console.log(
      `Calling webhook ${webhookUrl} (attempt ${retryAttempt + 1}/${webhookMaxRetries + 1})`
    );
...
async function directDatabaseUpdate(
  ...
  try {
    ...
    const { error } = await supabase.from(reportTable).update(updateData).eq('id', reportId);
...
async function runJob(
  ...
  try {
        ...
        .update({
          ...
          research_metadata: {
            ...
            model:
              ...
              process.env.VITE_PERPLEXITY_MODEL ||
              'sonar-pro',
            started_at: new Date().toISOString(),
          },
        })
    ...
    if (
      String(reportType).toLowerCase() === 'dynamic_questionnaire' ||
      (!reportTable && summaryIdForDynamic)
    ) {
      ...
      const { stagesJson, stagesObj } = await generateDynamicQuestionnaire(
        summaryIdForDynamic || ''
      );
    ...
    const mdl = normalizePerplexityModel(
      model || process.env.PERPLEXITY_MODEL || process.env.VITE_PERPLEXITY_MODEL || 'sonar-pro'
    );
    ...
    const SERVER_TIMEOUT_MS = isReasoningModel
      ? Math.min(110000, Math.floor(baseTimeout * 1.5))
      : baseTimeout;
    ...
    // Update progress
    await supabase.from('report_jobs').update({ percent: 80 }).eq('job_id', jobId);
    ...
    try {
      ...
      const reportId = (jobRow as any)?.metadata?.report_id as string | undefined;
      const reportTable = getReportTableFromType(reportType);
      const summaryIdForDynamic =
        (jobRow as any)?.metadata?.summary_id || (jobRow as any)?.summary_id;

      if (String(reportType).toLowerCase() === 'dynamic_questionnaire' && summaryIdForDynamic) {
        await notifyDynamicQuestionnaire(
          jobId,
          String(summaryIdForDynamic),
          {},
          'failed',
  ...
  if (req.method === 'POST') {
    try {
      const idempotencyKey =
        (req.headers['idempotency-key'] as string) || (req.body?.idempotency_key as string) || '';
