...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!payload.job_id || !payload.report_id || !payload.report_type) {
    ...
    return res
      .status(400)
      .json({ error: 'Missing required fields: job_id, report_id, report_type' });
  }
  ...
  try {
    ...
    if (existingReport.webhook_status === 'success') {
      ...
      return res
        .status(200)
        .json({ message: 'Webhook already processed successfully', report_id: payload.report_id });
    }
    ...
    console.log(
      `Successfully processed prelim-report webhook for job ${payload.job_id}, report ${payload.report_id}`
    );
