...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (
      existingReport.webhook_status === 'success' &&
      existingReport.research_status === 'completed'
    ) {
      await auditWebhookCall(
        payload.job_id,
        payload.report_id,
    ...
    console.log(
      `Successfully processed final-report webhook for job ${payload.job_id}, report ${payload.report_id}`
    );
