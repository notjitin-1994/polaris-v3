...
function inferDisplayType(sectionId: string, data: any): 'infographic' | 'markdown' | 'timeline' | 'table' {
	...
	if (sectionId === 'resources' || sectionId === 'risk_mitigation' || 'risks' in data || 'budget' in data) return 'table';
	if (sectionId === 'learning_objectives' || sectionId === 'assessment_strategy' || sectionId === 'success_metrics' || sectionId === 'target_audience' || 'objectives' in data || 'kpis' in data || 'metrics' in data || 'demographics' in data) return 'infographic';
	if (sectionId === 'instructional_strategy' || 'modalities' in data) return 'infographic';
	if (sectionId === 'sustainability_plan' || (data?.content && (data?.maintenance_schedule || data?.scaling_considerations))) return 'infographic';
