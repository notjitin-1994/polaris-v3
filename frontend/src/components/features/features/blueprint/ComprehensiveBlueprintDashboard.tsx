...
export function ComprehensiveBlueprintDashboard({
  ...
  return (
    ...
    >
      {/* Enhanced Header */}
      <motion.div
        variants={shouldReduceAnimations ? undefined : itemVariants}
          ...
          <div className="space-y-4">
            {modules.map((module, index) => (
              <div key={index} className="glass-strong rounded-xl border border-white/10 p-6">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {module.topics.map((topic, tIndex) => (
                        <span
                          key={tIndex}
                          className="bg-primary/90 rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-900"
                        >
                          {topic}
                        </span>
                      ))}
                ...
                {module.assessment && (
                  <div className="border-success/30 bg-success/20 rounded-lg border p-4">
                    <p className="mb-1 text-sm font-bold text-slate-900">
                      <span className="bg-success rounded px-2 py-1 text-xs font-bold text-slate-900">
                        Assessment
                      </span>
                      <span className="ml-2">{module.assessment.type}</span>
                    </p>
                    <p className="text-foreground text-sm">{module.assessment.description}</p>
                  </div>
                )}
              </div>
            ))}
              ...
              {blueprint.instructional_strategy.modalities.map((modality, index) => (
                <div key={index} className="glass rounded-xl border border-white/10 p-4">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className="bg-primary rounded-full px-3 py-1 text-sm font-bold text-slate-900">
                  ...
                  {modality.tools && modality.tools.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {modality.tools.map((tool, tIndex) => (
                        <span
                          key={tIndex}
                          className="bg-primary/90 rounded-full px-3 py-1 text-xs font-semibold text-slate-900"
                        >
                          {tool}
                        </span>
                      ))}
...
function SectionCard({
  ...
  return (
    <motion.div
      ...
      className="glass-card overflow-hidden rounded-3xl border border-white/10"
    >
