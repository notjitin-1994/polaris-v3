...
CREATE INDEX idx_payments_user_history ON public.payments(user_id, payment_date DESC)
  WHERE status IN ('captured', 'refunded');

-- Index for failed payments analysis
CREATE INDEX idx_payments_failed ON public.payments(user_id, error_code)
  WHERE status = 'failed';
