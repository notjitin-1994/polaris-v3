...
CREATE INDEX idx_webhook_events_processing_status ON public.webhook_events(processing_status);
CREATE INDEX idx_webhook_events_created_at ON public.webhook_events(created_at DESC);
CREATE INDEX idx_webhook_events_related_subscription_id ON public.webhook_events(related_subscription_id);
CREATE INDEX idx_webhook_events_related_payment_id ON public.webhook_events(related_payment_id);

-- Index for unprocessed webhooks
CREATE INDEX idx_webhook_events_unprocessed ON public.webhook_events(created_at DESC)
  WHERE processing_status IN ('pending', 'failed');

-- Index for signature verification
CREATE INDEX idx_webhook_events_unverified ON public.webhook_events(created_at DESC)
...
-- (they should use the subscriptions and payments tables instead)
